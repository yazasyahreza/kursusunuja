...
@section('this-page-content')
    <div class="container-fluid p-0">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h1 class="h3 mb-0">Edit <strong>Peserta Kursus</strong></h1>
            ...
            <div class="col-12">
                <div class="card flex-fill">
                    <div class="card-header">
                        <h5 class="card-title mb-0">Form Edit Peserta Kursus</h5>
                    </div>
                    ...
                    <div class="card-body">
                        ...
                        <form action="{{ route('master.peserta-kursus.update', $data->id_peserta_kursus) }}" method="POST">
                            @csrf
                            @method('PUT')
                                    ...
                                    <select name="kursus_id" id="kursus_id"
                                        ...
                                        @foreach ($kursuses as $kursus)
                                            <option value="{{ $kursus->id_kursus }}"
                                                {{ old('kursus_id', $data->kursus_id) == $kursus->id_kursus ? 'selected' : '' }}>
                                                {{ $kursus->judul }}
                                            </option>
                                        @endforeach
                                    </select>
                                    ...
                                    <select name="peserta_id" id="peserta_id"
                                        ...
                                        @foreach ($pesertas as $peserta)
                                            <option value="{{ $peserta->id_peserta }}"
                                                {{ old('peserta_id', $data->peserta_id) == $peserta->id_peserta ? 'selected' : '' }}>
                                                {{ $peserta->user->name }}
                                            </option>
                                        @endforeach
                                    </select>
